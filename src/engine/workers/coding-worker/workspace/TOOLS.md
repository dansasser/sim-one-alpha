...
For asynchronous connectors such as Telegram, pass the trusted current `eventId` to `github_auth_status` and `github_auth_start`; when continuing an approved login, also pass the `approvalRequestId` returned for that login. These are the only model-visible routing values; trusted ingress authority is stored server-side and bound to the Flue agent instance plus that event ID. Never invent or substitute either value from an unrelated request or conversation.

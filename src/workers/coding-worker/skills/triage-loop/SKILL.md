...
- Produce an explicit plan (`CodingPlanItem[]`) covering triage, implementation, verification, review, and GitHub stages when relevant.
- Return the result through the `coding_triage_submit_result` tool as a `CodingTriageResult` with `plan`, `filesToInspect`, and `recommendedExecutionPath`.

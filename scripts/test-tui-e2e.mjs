...
const modelEnv = {
  OLLAMA_API_KEY: process.env.OLLAMA_API_KEY || envFileValues.OLLAMA_API_KEY || 'tui-e2e-test-key',
  CODEX_BRAIN_LOCAL_API_KEY:
    process.env.CODEX_BRAIN_LOCAL_API_KEY || envFileValues.CODEX_BRAIN_LOCAL_API_KEY || 'tui-e2e-test-key',
...
try {
  ...
  // Test 1: Direct agent prompt (simulates TUI sendMessage path)
  ...
  // Test 2: Verify the response is a real agent response, not an error
  assertJson(
    !result1.isError && !result1.result?.isError,
    `agent response should not be an error. Got: ${JSON.stringify(result1).slice(0, 500)}`,
  );

  // Test 3: Verify event stream shows the prompt was processed
  console.log('[tui-e2e] Test 3: Checking event stream for prompt processing...');
  const eventsResponse = await fetch(
    `${baseUrl}/runs/agents%2Forchestrator%2Ftui-e2e-1`,
    {
      method: 'GET',
  ...
  if (eventsResponse.status === 200) {
    ...
    assertJson(
      Array.isArray(eventsData.events) && eventsData.events.length > 0,
      `event stream should have events for the session. Got: ${JSON.stringify(eventsData).slice(0, 500)}`,
    );
    const eventTypes = eventsData.events.map((e) => e.type);
    assertJson(
      eventTypes.includes('operation') || eventTypes.includes('operation_start'),
      `event stream should include operation events. Types: ${eventTypes.join(', ')}`,
    );
    console.log('[tui-e2e] Test 3 PASSED: event stream has operation events');
  } else {
    console.log(`[tui-e2e] Test 3 SKIPPED: event stream endpoint returned ${eventsResponse.status} (may not be available in test mode)`);
  }

  // Test 4: Verify CLI binary exists and can be invoked
  console.log('[tui-e2e] Test 4: Verifying CLI binary is runnable...');
  const cliResult = await runCliCommand(['--help']);
  assertEqual(cliResult.exitCode, 0, 'CLI --help should exit 0');
  assertJson(
    cliResult.stdout.length > 0,
    'CLI --help should produce output',
  );
  console.log('[tui-e2e] Test 4 PASSED: CLI binary is runnable');

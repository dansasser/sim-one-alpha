...
Run `pnpm test` and verify TypeScript reports that `src/workers/researcher/researcher.ts` does not exist.
...
Run `pnpm test` and verify the new subagent test passes.
...
Run `pnpm test` and verify the assertion fails because only `coding_worker` is registered.
...
Run `pnpm test` and verify the orchestrator config test passes.
...
Run `pnpm test` and verify TypeScript reports that `src/workflows/research.ts` does not exist.
...
Run `pnpm test` and verify the research workflow prompt test passes.

...
`chat_sessions.display_name` stores only names explicitly supplied through `/rename`, `/new <title>`, or `/clear <title>`. It is separate from the existing prompt-derived conversation `title`. Session-management responses expose the explicit display name for `/session` and `/resume`, while ordinary agent responses omit prompt-derived titles. Ratatui renders `SIM-ONE Alpha - <session-id-or-name>` in the transcript border and keeps the status row focused on runtime state. The transcript command response remains human-readable, but state synchronization never parses that response text.
...
The transcript border is reserved for product and session identity. It shows the durable id until an explicit display name exists and never includes live-tail state. Live-tail is implicit; manual scrollback adds `view: scrolled back` to the status row until the viewport returns to the tail.

...
Session-management responses carry an explicit title only when the user names a session through `/rename`, `/new <title>`, or `/clear <title>`. Ordinary agent responses omit prompt-derived internal titles. Ratatui starts the status label with the session id, replaces that one field with a confirmed explicit name, and clears it back to the new id when switching to an unnamed session. The transcript command response remains human-readable, but state synchronization never parses that response text.

...
src/routes/chat-events.ts
  ...
  Does not call c.executionCtx, /workflows/chat for normal chat execution, or a non-Flue orchestrator.
...
src/workflows/chat.ts
  ...
  Exposes its Flue HTTP route through imported API-secret middleware for finite workflow and local compatibility paths. It is not the primary /api/chat/events execution boundary.

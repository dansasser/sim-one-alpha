...
Tools are executable, model-callable capabilities in GOROMBO Agent. They are implemented with Flue's `defineTool(...)` and attached only to the agents that should own them.
